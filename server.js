...
app.get('/api/video-info', async (req, res) => {
    try {
        ...
        info.formats.forEach(format => {
            if (format.ext === 'mp4' && format.vcodec !== 'none' && format.acodec !== 'none' && format.height) {
        ...
        // Filter audio formats (no conversion needed!)
        ...
        if (formats.mp3.length === 0 && formats.mp4.length > 0) {
            formats.mp3.push({
                quality: '128kbps',
                formatId: formats.mp4[0].formatId,
                ext: 'mp4',
...
app.get('/api/download', async (req, res) => {
    try {
        ...
        const ytDlpCmd = `"${ytDlpPath}" -f ${formatId} -o "${outputPath}" "${url}"`;
        console.log('Running command:', ytDlpCmd);
